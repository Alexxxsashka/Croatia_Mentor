...
export default function GamesPage() {
  ...
  return <>
    ...
    <div className="game-grid">{games.map(game=>{const Icon=game.icon;return <Link key={game.id} href={`/games/${game.id}`} className="game-card"><span className="game-cover"><Image src={`/assets/learning/${["reading"].includes(game.id)?"travel":["listening","audio-spelling"].includes(game.id)?"audio":["fill-blanks","scramble"].includes(game.id)?"grammar":"words"}.png`} alt="" fill sizes="(max-width: 700px) 90vw, 400px" /></span><div className="between"><Icon className="game-icon" size={28} strokeWidth={1.6}/><span>{game.xp}</span></div><h2>{game.title}</h2><p>{game.description}</p><div className="between"><span>{game.cta}</span><ArrowRight size={18}/></div></Link>})}</div>
  </>;
}
